...
There are a number of different command line arguments. (Please check out the utility directly - I may make changes and forget to keep the following up to date.)
        ...
        Note names can be note numbers (60), note names (G#3), or button names from the following list.
...
`--fps` is the target frames per second for the animation, which is the rate positional events will be sent to Character Animator. 24 is high accuracy, but Character Animator on my laptop could not keep up so the default is set to 8. With the webcam on in Character Animator I had to set this to a value like 4. Higher numbers will result in smoother motions, if Character Animator can keep up.

### Movement Bounding Box

`--min-x`, `--max-x`, `--min-y`, `--max-y` set the min and max raw values generated by the HTC Vive position values. The default values worked well for the desk I was sitting at - you will almost certainly have to adjust these values for your setup based on where you plan to stand. The values are in meters. Values in this range are scalled to MIDI values in the range 0 to 127.
...
Some ideas for future work are as follows.
 ...
 - Add support for multiple triggers based on how far trigger is squeezed to allow multiple hand positions (e.g. open, pointing, and closed).
 - It might be better if min/max were autocalibrated - e.g. have the code work out the max X/Y values it sees you use, when a button is pressed so you can move around without accidental recalibration.
 - Add a new option to send dummy control events so you can bind these controls more easily, like triggers and notes, without a HTC Vive present.
 - Explore better calibration where you could click certain reference points and have the system work out the numbers for you, especially for Position X/Y values within the control panel. E.g. want to have hands appear directly next to body
